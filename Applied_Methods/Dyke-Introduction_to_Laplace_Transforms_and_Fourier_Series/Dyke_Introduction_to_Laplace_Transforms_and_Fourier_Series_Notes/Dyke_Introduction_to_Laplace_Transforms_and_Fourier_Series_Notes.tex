...
For a function $F(x)$ to have a Laplace transform, it must be of exponential order and be piecewise continuous on the interval $[0,\infty)$. To be of ``exponential order`` means that there exists an $M,\alpha \in \R$ and $M,\alpha >0$ such that $|F(x)| \leq Me^{\alpha t}$. We also assume that the function is Riemann integrable. We shall now begin by proving some basic properties. Phil Dyke adopts the convention 
\begin{equation*}
    \lapt{F(t)} = f(s)
\end{equation*}
